...
/**
 * @file plugins/importexport/colobok/COLOBOKExportDom.inc.php
 *
 * Copyright (c) 2003-2013 Artem Gusarenko
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class COLOBOKExportDom
...
class colobokExportDom {
	 ...
	 */
	function &generateJournalDom(&$doc, &$journal, &$issue) {
		...
		//$pubArticles =& $pubArticleDao->getPublishedArticles($issue->getNaumber());
		//var_dump($pubArticles);
		while ($pubArticle =& $pubArticles->next()) {
			$issue1 =& $issueDao->getIssueById($pubArticle->getIssueId());
			if ($issue1 == $issue){
				//$issue =& $issueDao->getIssueById($pubArticle->getIssueId());
				if(!$issue) continue;
				$section =& $sectionDao->getSection($pubArticle->getSectionId());
	
				$articleNode =& colobokExportDom::generateArticleDom($doc, $journal, $issue, $section, $pubArticle);

				XMLCustomWriter::appendChild($records, $articleNode);

				//unset($issue, $section, $articleNode);
				unset($section, $articleNode);
			}
		}
	...
	function &generateArticleDom(&$doc, &$journal, &$issue, &$section, &$article) {
		...
		/* --- Article Language --- */
		XMLCustomWriter::createChildWithText($doc, $root, 'language', colobokExportDom::mapLang($article->getLanguage()), false);
		...
		XMLCustomWriter::createChildWithText($doc, $root, 'publicationDate', colobokExportDom::formatDate($issue->getDatePublished()), false);
		...
		foreach ((array) $article->getTitle(null) as $locale => $title) {
			...
			if (strlen($locale) == 5) XMLCustomWriter::setAttribute($titleNode, 'language', colobokExportDom::mapLang(String::substr($locale, 0, 2)));
		}
		...
		$affilList = colobokExportDom::generateAffiliationsList($article->getAuthors());

		foreach ($article->getAuthors() as $author) {
			$authorNode =& colobokExportDom::generateAuthorDom($doc, $root, $issue, $article, $author, $affilList);
		...
		foreach ((array) $article->getAbstract(null) as $locale => $abstract) {
			...
			if (strlen($locale) == 5) XMLCustomWriter::setAttribute($abstractNode, 'language', colobokExportDom::mapLang(String::substr($locale, 0, 2)));
		}
